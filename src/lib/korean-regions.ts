// 대한민국 행정구역 (시도 + 시군구) — 2026 기준
// 정책의 sido_scope / sigungu_scope 와 일치해야 하므로 이 파일 명칭이 표준이 됩니다.
...
export const SIGUNGU_MAP: Record<Sido, string[]> = {
  서울특별시: [
...
export function isValidRegion(sido: string, sigungu: string): boolean {
  if (!(SIDO_LIST as readonly string[]).includes(sido)) return false
  return SIGUNGU_MAP[sido as Sido].includes(sigungu)
}
